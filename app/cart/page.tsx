              ...
              {cart.lines.nodes.map((line) => (
                <div key={line.id} className="rounded-[24px] border border-[#111111]/10 bg-white p-4">
                  <div className="flex items-center justify-between gap-4">
                    ...
                    <button type="button" onClick={() => removeItem(line.id)} className="text-sm font-medium text-[#666666] underline-offset-4 hover:underline">{t("Remove")}</button>
                  </div>
                  <div className="mt-4 flex items-center justify-between">
                    <div className="flex items-center gap-2 rounded-full border border-[#111111]/10 px-2 py-1">
                      <button type="button" className="h-6 w-6 text-lg" onClick={() => updateItemQuantity(line.id, Math.max(0, line.quantity - 1))} aria-label={t("Decrease quantity")}>−</button>
                      <span className="min-w-6 text-center text-sm font-medium">{line.quantity}</span>
                      <button type="button" className="h-6 w-6 text-lg" onClick={() => updateItemQuantity(line.id, line.quantity + 1)} aria-label={t("Increase quantity")}>+</button>
                    </div>
